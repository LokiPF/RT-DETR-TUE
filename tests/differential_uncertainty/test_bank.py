...
def test_bank_filters_padded_queries_and_low_confidence_queries():
    reservoir = Reservoir(capacity=3, dimension=2, seed=44)
    record = {
        "persistence": torch.tensor([[1.0, 1.0], [2.0, 2.0], [3.0, 3.0]]),
        "logits": torch.tensor([[0.0, 0.0], [2.0, 0.0], [5.0, 0.0]]),
    ...
    torch.testing.assert_close(reservoir.bank().float(), torch.tensor([[1.0, 1.0], [2.0, 2.0], [4.0, 4.0]]))
