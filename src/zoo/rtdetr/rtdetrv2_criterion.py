...
class RTDETRCriterionv2(nn.Module):
    ...
    def gaussian_box_nll(self, outputs, targets, indices, num_boxes):
        batch_indices = torch.cat(
            [
                torch.full_like(src_indices, batch_index)
                for batch_index, (src_indices, _) in enumerate(indices)
            ]
        )
        query_indices = torch.cat([src_indices for src_indices, _ in indices])

        predicted_boxes = outputs["pred_boxes"][batch_indices, query_indices]
        predicted_logvars = outputs["pred_boxes_logvars"][
            batch_indices, query_indices
        ].clamp(min=-10.0, max=4.0)
        ...
        residual = predicted_boxes - target_boxes

        nll = 0.5 * (
            torch.exp(-predicted_logvars) * residual.square() + predicted_logvars
        )

        return {"loss_nll": nll.sum() / max(float(num_boxes), 1.0)}
