...
class TransformerDecoder(nn.Module):
    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            ...
            box_logvars = bbox_logvar_heads[i](output)

            if self.training:
                dec_out_logits.append(score_head[i](output))
                logvars_by_layer.append(box_logvars)
                if i == 0:
        ...
        return (
            ...
            torch.stack(logvars_by_layer),
        )


@register()
class RTDETRTransformerv2ProbHead(nn.Module):
    ...
    def __init__(
        ...
        self.dec_bbox_logvar_head = nn.ModuleList(
    ...
    def _reset_parameters(self):
        ...
        for logvar_head in self.dec_bbox_logvar_head:
            nn.init.zeros_(logvar_head.weight)

            # Initial std ≈ exp(-6 / 2) ≈ 0.05 in normalized coordinates.
            nn.init.constant_(logvar_head.bias, -6.0)
    ...
    def forward(self, feats, targets=None):
        ...
        # decoder
        out_bboxes, out_logits, output_logvars = self.decoder(
            init_ref_contents,
            init_ref_points_unact,
            memory,
            spatial_shapes,
            self.dec_bbox_head,
            self.dec_bbox_logvar_head,
            self.dec_score_head,
            self.query_pos_head,
            attn_mask=attn_mask,
        )
        ...
        if self.training and dn_meta is not None:
            ...
            denoising_logvars, output_logvars = torch.split(
                output_logvars,
        ...
        out = {
            ...
            "pred_boxes_logvars": output_logvars[-1],
        }
    ...
    @torch.jit.unused
    def _set_aux_loss(self, logits, boxes, logvars=None):
        outputs = []

        for layer_index, (layer_logits, layer_boxes) in enumerate(zip(logits, boxes)):
            item = {
                "pred_logits": layer_logits,
                "pred_boxes": layer_boxes,
            }

            if logvars is not None:
                item["pred_box_logvars"] = logvars[layer_index]
