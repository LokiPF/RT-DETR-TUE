    ...
    for batch in tqdm(list(chunks(items, BATCH)), desc=d["name"]):
        ...
        for conf, unc, cor, dropped in per_image_stats:
            ...
            if not conf:
                n_empty += 1; continue
            p["dc"] += conf; p["du"] += unc
            p["ic"].append(mean(conf)); p["iu"].append(mean(unc))
            cc = [x for x, k in zip(conf, cor) if k]; cu = [x for x, k in zip(unc, cor) if k]
            if cc:
                p["dcc"] += cc; p["duc"] += cu
                p["icc"].append(mean(cc)); p["iuc"].append(mean(cu))
    print(f"[{'OOD' if d['ood'] else 'ID '}] {d['name']}: {len(p['ic'])} imgs w/ dets, "
...
def main():
    ...
    res = {}
    idp = {k: [] for k in ["dc", "du", "ic", "iu", "dcc", "duc", "icc", "iuc"]}  # shared ID pool
    for ds in datasets:
        d = ds["dataset"]
    ...
    variants = [
        ("auroc_det_unc",     "du", "du", True),  ("auroc_det_1conf",     "dc", "dc", False),
        ("auroc_img_unc",     "iu", "iu", True),  ("auroc_img_1conf",     "ic", "ic", False),
        ("auroc_detC_unc",    "duc", "duc", True), ("auroc_detC_1conf",   "dcc", "dcc", False),
        ("auroc_imgC_unc",    "iuc", "iuc", True), ("auroc_imgC_1conf",   "icc", "icc", False),
    ]
    out_csv = output_dir / "benchmark.csv"
    with open(out_csv, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["dataset", "split", "n_img", "n_det", "n_det_correct"]
                   + COCO_STATS + [c for c, *_ in variants])
        for name, v in res.items():
            p = v["p"]
            a = ([f"{auroc(idp[nk], p[pk], hi):.4f}" for _, nk, pk, hi in variants]
                 if v["ood"] else [""] * len(variants))
            w.writerow([name, "OOD" if v["ood"] else "ID",
                        len(p["ic"]), len(p["dc"]), len(p["dcc"])]
                       + [f"{s:.4f}" for s in v["stats"]] + a)
    print("saved:", out_csv)
